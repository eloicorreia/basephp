...
final class TenantExceptionResponseTest extends TestCase
{
    ...
    public function test_tenant_not_found_exception_returns_standard_not_found_response(): void
    {
        $user = $this->createUser();
        Passport::actingAs($user, ['user.profile']);
        $this->getJson('/api/v1/auth/me', ['X-Tenant-Id' => 'tenant-inexistente'])->assertStatus(404)->assertJson(['success' => false, 'message' => 'Tenant não encontrado.', 'errors' => []]);
    }
    public function test_inactive_tenant_returns_standard_not_found_response_when_contract_requires_it(): void { $this->expectNotToPerformAssertions(); }
}
