...
final class ExceptionHandlingAndSystemLogTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Route::middleware(['api', 'auth:api'])->prefix('api/v1/test')->group(function (): void {
            ...
            Route::post('/validation-error', function (Request $request): JsonResponse {
    ...
    public function test_it_persists_system_log_with_request_and_trace_ids(): void
    {
        $bundle = $this->tenantBundle();
        $requestId = (string) Str::uuid();
        $traceId = (string) Str::uuid();
    ...
    public function test_it_persists_system_log_with_authenticated_user_id(): void
    {
        $bundle = $this->tenantBundle();
        $requestId = (string) Str::uuid();
        Passport::actingAs($bundle['user'], ['user.profile', 'tenant.access', 'admin.full', 'user.password.change']);
